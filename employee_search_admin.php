...
<html>
    ...
    <div>
        ...
        <select onchange="location = this.value;">
            ...
            <option value="employee_search_name.php">Name</option>
            <option value="employee_search_eid.php">EID</option>
            <option value="employee_search_admin.php">Admin</option>
        </select>
        </form>
        <form  name = "search" action="" method="post">
            <label for="eid">Admin</label>
            <select name="admin">
    ...
    <?php
    if(isset($_POST["search"])and isset($_POST["admin"])){
        $as = 0;
        if ($_POST["admin"] == "yes") {
            $as = 1;
        }
        $userconn = new mysqli($GLOBALS["servername"], $GLOBALS["username"], $GLOBALS["dbpassword"], $GLOBALS["usersdb"]);
        $sql = "SELECT * FROM employee WHERE Admin_Status = $as ORDER BY Fname, Lname";
