...
<html>
    ...
    <div>
        <form action="" method="post">
            ...
            <label for="police">Police Response</label>
            <select name="police">
    ...
    if(isset($_POST["add"])){
        if((is_numeric($_POST["sku"])) and (strlen($_POST["sku"])== 7) and ((checkSKU($_POST["sku"])== true))){
            if((is_numeric($_POST["amount"])) and ($_POST["amount"] > 0)){
                    if((is_numeric($_POST["cost"])) and ($_POST["cost"] > 0)){
                        $sku = (int)$_POST["sku"];
                        ...
                        $sql = "INSERT INTO stolen VALUES ($sku, $amount, $police, '$date[year]-$date[mon]-$date[mday]', $cost)";
                        if($userconn->query($sql)==true){
                            echo $sku." has been reported";
                            $sql = "UPDATE in_stock SET no_instock = (no_instock-$amount)WHERE sku = $sku"; 
                            if($userconn->query($sql)==true){
                                
                            }
                        }else{
                            echo "Item has not been reported. Please try again.";
                        }
                        $userconn->close();
                    }
            }else{
