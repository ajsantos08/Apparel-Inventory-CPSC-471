...
<html>
    ...
    if(isset($_POST["add"])){
        if((is_numeric($_POST["eid"])) and (strlen($_POST["eid"])== 5) and ((checkEID($_POST["eid"])== false))){
            if((strlen($_POST["password"])> 5) and (strlen($_POST["password"])< 31)){
                if((ctype_alpha($_POST["fname"])) and (ctype_alpha($_POST["lname"])) and (strlen($_POST["fname"])< 31) and (strlen($_POST["fname"])> 1) and (strlen($_POST["lname"])< 31) and (strlen($_POST["lname"])> 1)){
                    $admin = 0;
                    ...
                    $fname =  (string) ucfirst($_POST["fname"]);
                    $lname = (string) ucfirst($_POST["lname"]);
                    $userconn = new mysqli($GLOBALS["servername"], $GLOBALS["username"], $GLOBALS["dbpassword"], $GLOBALS["usersdb"]);
                    $sql = "INSERT INTO employee (Employee_ID, Password, Fname, Lname, Admin_Status) VALUES ($eid, '$password','$fname','$lname', $admin)";
                    if($userconn->query($sql)==true){
                        echo $eid." ".$fname." ".$lname." has been added to the database";
                    }else{
                        echo "Employee was not added. Please try again.";
                    }
                    $userconn->close();
                }else{
                   echo "Invalid Name"; 
